...
function handleTrade(event) {
  ...
  if (!currencyId) {
    return showToast("Lütfen bir döviz seç.", "error");
  }

  if (!tradingWindowOpen()) {
    return showToast("Döviz işlemleri 09:00 - 21:00 arasında yapılabilir.", "error");
  }

  const currency = defaultCurrencies.find((item) => item.id === currencyId);
  const qty = Number(amount);
  if (!qty || qty <= 0) {
    return showToast("Geçerli bir adet gir.", "error");
  }
  ...
  if (action === "buy") {
    if (state.gameplay.balances.cash < price) {
      return showToast("Bakiyeniz yetersizdir!", "error");
    }
    ...
    if (holdings < qty) {
      return showToast("Yeterli adet yok!", "error");
    }
    state.gameplay.balances.cash += price;
    state.gameplay.currencies[currencyId].holdings = holdings - qty;
    ...
    return showToast("İşlem seçiniz.", "error");
  }
...
function handleListing(event) {
  ...
  if (state.gameplay.inventory[item] < qty) {
    return showToast("Envanterde yeterli adet yok!", "error");
  }
  ...
  showToast("İlan pazar yerine taşındı.");
  event.target.reset();
  renderInventory();
  renderMarket();
  saveState();
...
function handleWork() {
  ...
  showToast(`${earned.toLocaleString("tr-TR")} adet demir üretildi.`);
  renderSummary();
  renderInventory();
  saveState();
}
...
function attachEvents() {
  ...
  registerForm.addEventListener("submit", (event) => {
    ...
    if (exists) {
      return showToast("Bu kullanıcı adı kullanımda.", "error");
    }
    const newUser = {
      ...payload,
      level: 1,
