...
function renderInventory() {
      ...
      .map(([key, value]) => {
        ...
        return `
          <article class="inventory-card">
            <div class="inventory-icon">${visual.icon}</div>
            <p class="inventory-name">${visual.label}</p>
            <p class="inventory-qty">${value.toLocaleString("tr-TR")} adet</p>
          </article>
        `;
      })
      .join("");
  ...
  const sellable = entries.filter(([, value]) => Number(value) > 0);
  if (!sellable.length) {
    dom.listingSelect.innerHTML =
      '<option value="" disabled selected>Satılabilir eşya yok</option>';
  } else {
    dom.listingSelect.innerHTML = sellable
      .map(([key]) => `<option value="${key}">${formatItem(key)}</option>`)
      .join("");
  }
  const submitBtn = dom.listingForm.querySelector('button[type="submit"]');
  if (submitBtn) {
    submitBtn.disabled = sellable.length === 0;
  }
}
    ...
    .map((listing) => {
      ...
      return `
        <article class="market-card" data-listing="${listing.id}">
          ...
          <div class="market-icon">${visual.icon}</div>
          <h4>${visual.label}</h4>
          <p class="muted">Satıcı: ${listing.seller}</p>
          <p>Adet: ${availableQty.toLocaleString("tr-TR")}</p>
          <p>Birim: ${formatCurrency(unit)}</p>
          <p>Toplam: <strong>${formatCurrency(total)}</strong></p>
          ${
            owned
