...
const dom = {
  ...
  farmNavStack: document.getElementById("farm-nav-stack"),
  ...
  cowVipState: document.getElementById("cow-vip-state"),
  cowCommission: document.getElementById("cow-commission-range"),
  cowSecurity: document.getElementById("cow-security"),
  cowBulkButton: document.getElementById("cow-bulk-button"),
  cowBulkSection: document.getElementById("cow-bulk-block"),
  cowEmptyNotice: document.getElementById("cow-none-message"),
  cowList: document.getElementById("cow-list"),
  cowShopButton: document.getElementById("cow-purchase-button"),
  cowShopView: document.querySelector('[data-farm-view="cow-shop"]'),
  cowShopCooldown: document.getElementById("cow-shop-cooldown"),
  cowShopNotice: document.getElementById("cow-shop-notice"),
  cowBuyGem: document.getElementById("cow-buy-gem"),
  cowBuyTcoin: document.getElementById("cow-buy-tcoin"),
  cowTypes: document.getElementById("cow-types"),
  cowPurchaseActions: document.getElementById("cow-purchase-actions"),
};
...
const COW_SLOT_COST = 3;
const COW_LIFETIME = 90 * 24 * 60 * 60 * 1000;
const COW_MILK_COOLDOWN = 3 * 60 * 60 * 1000;
...
function getCowType(id) {
  return (
    COW_TYPES.find((type) => type.id === id) || COW_TYPES.find((type) => type.id === "montofon")
  );
}

function createCowId() {
  return `cow-${Date.now().toString(36)}-${Math.random().toString(36).slice(2, 6)}`;
}

function sanitizeCows() {
  const now = Date.now();
  state.gameplay.cows = (state.gameplay.cows || [])
    .map((cow) => {
      const type = getCowType(cow.type);
      return {
        id: cow.id || createCowId(),
        type: type.id,
        acquiredAt: cow.acquiredAt || now,
        expiresAt: cow.expiresAt || now + COW_LIFETIME,
        grazingUntil: cow.grazingUntil || null,
        lastMilkAt: cow.lastMilkAt || null,
      };
    })
    .filter((cow) => cow.expiresAt > now);
}

function farmUsage() {
  const farm = state.gameplay.farm || defaultGameplay().farm;
  const planted = farm.plantedArea || 0;
  const cows = state.gameplay.cows?.length || 0;
  const used = planted + cows * COW_SLOT_COST;
  return { used, capacity: farm.size };
}
...
function refreshCowQuestion() {
  currentCowQuestion = generateCowQuestion();
  if (dom.cowSecurity) {
    dom.cowSecurity.value = "";
    dom.cowSecurity.placeholder = currentCowQuestion.prompt;
  }
}
...
function cowCanBeMilked(cow) {
  const now = Date.now();
  if (!cow) return false;
  if (cow.grazingUntil && cow.grazingUntil > now) return false;
  return true;
}

function cowYieldRange(cow, applyCommission = false) {
  const type = getCowType(cow.type);
  const vipMultiplier = state.gameplay.vipActive ? 2 : 1;
  const min = type.range[0] * vipMultiplier;
  const max = type.range[1] * vipMultiplier;
  const factor = applyCommission ? 0.9 : 1;
  return {
...
function renderCowCommission() {
  if (!dom.cowCommission) return;
  const totals = state.gameplay.cows.reduce(
    (acc, cow) => {
      const range = cowYieldRange(cow, true);
      acc.min += range.min;
      acc.max += range.max;
      return acc;
    },
  ...
  const text = `${totals.min.toLocaleString("tr-TR")} ile ${totals.max.toLocaleString(
    "tr-TR"
  )} Litre Arası Süt`;
  dom.cowCommission.textContent = text;
  document.querySelectorAll(".cow-commission-inline").forEach((node) => {
...
function renderCows() {
  ...
  sanitizeCows();
  const { used, capacity } = farmUsage();
  if (dom.cowCapacity) {
    dom.cowCapacity.textContent = `${capacity.toLocaleString("tr-TR")} M2 / ${used.toLocaleString(
      "tr-TR"
    )} M2`;
  }
  if (dom.cowCount) {
    dom.cowCount.textContent = `${state.gameplay.cows.length}`;
  }
  if (dom.cowVipState) {
    dom.cowVipState.textContent = state.gameplay.vipActive
      ? "Vip 2X Tahsilat Aktif"
      : "Vip 2x Tahsilat Pasif";
    dom.cowVipState.classList.toggle("positive", state.gameplay.vipActive);
    dom.cowVipState.classList.toggle("negative", !state.gameplay.vipActive);
  }
  renderCowCommission();
  const milkable = state.gameplay.cows.filter((cow) => cowCanBeMilked(cow));
  const showBulk = milkable.length > 0;
  if (dom.cowBulkSection) {
    dom.cowBulkSection.classList.toggle("hidden", !showBulk);
  }
  if (dom.cowEmptyNotice) {
    dom.cowEmptyNotice.classList.toggle("hidden", showBulk);
  }
  if (showBulk && !currentCowQuestion) {
    refreshCowQuestion();
  }

  dom.cowList.innerHTML = state.gameplay.cows
    .map((cow) => {
      const type = getCowType(cow.type);
      const range = cowYieldRange(cow, false);
      const grazing = cow.grazingUntil && cow.grazingUntil > Date.now();
      const remaining = grazing ? cow.grazingUntil - Date.now() : 0;
      const hours = Math.floor(remaining / (1000 * 60 * 60));
      const minutes = Math.floor((remaining % (1000 * 60 * 60)) / (1000 * 60));
      const seconds = Math.floor((remaining % (1000 * 60)) / 1000);
      const expiresLabel = formatDateTime(cow.expiresAt);
      const milkButton = grazing
        ? `<button class="btn-warn" disabled>${String(hours).padStart(2, "0")}:${String(
            minutes
          ).padStart(2, "0")}:${String(seconds).padStart(2, "0")}</button>`
        : `<button class="btn-positive" data-milk="${cow.id}">İneği Sağ</button>`;
      return `
        <article class="cow-card" data-cow="${cow.id}">
          <div class="cow-visual">${type.icon}</div>
          <div class="cow-body">
            <h4>${escapeHtml(type.name)}</h4>
            <p>Üretim: <strong>${range.min.toLocaleString("tr-TR")} / ${range.max.toLocaleString("tr-TR")}</strong> Litre</p>
            <p>Ömrü: <strong>${expiresLabel}</strong></p>
            <p class="status ${grazing ? "muted" : "positive"}">${
        grazing ? "Otlanıyor" : "Otlanmıyor"
      }</p>
            ${milkButton}
          </div>
        </article>
  ...
  dom.cowList.querySelectorAll("[data-milk]").forEach((btn) => {
    btn.addEventListener("click", handleSingleMilk);
  });
  renderCowShop();
}
...
function openPlantModal() {
  ...
  const usedByCows = (state.gameplay.cows?.length || 0) * COW_SLOT_COST;
  const available = Math.max(farm.size - usedByCows, 0);
  dom.plantModal.classList.remove("hidden");
  if (dom.plantInput) {
    dom.plantInput.placeholder = `Max: ${available} M2`;
...
function handlePlantConfirm() {
  ...
  const usedByCows = (state.gameplay.cows?.length || 0) * COW_SLOT_COST;
  if (amount + usedByCows > farm.size) {
    return showDialog("İnekler nedeniyle yeterli alan yok.", "error");
  }
  if (amount > farm.size) {
...
function handleSingleMilk(event) {
  const cowId = event.target.dataset.milk;
  const cow = state.gameplay.cows.find((entry) => entry.id === cowId);
  if (!cow) return;
  if (!cowCanBeMilked(cow)) {
    return showDialog("İnek henüz otlanıyor.", "error");
  }
  const yieldRange = cowYieldRange(cow, false);
  const reward = randomBetween(yieldRange.min, yieldRange.max);
  const now = Date.now();
  cow.grazingUntil = now + COW_MILK_COOLDOWN;
  cow.lastMilkAt = now;
  state.gameplay.inventory.sut =
    (state.gameplay.inventory.sut || 0) + reward;
  renderInventory();
  renderCows();
  showDialog(`${reward.toLocaleString("tr-TR")} litre süt topladın.`, "success");
  saveState();
}

function handleBulkMilk() {
  if (!currentCowQuestion) refreshCowQuestion();
  const answer = Number(dom.cowSecurity?.value || 0);
  if (!currentCowQuestion || answer !== currentCowQuestion.answer) {
    showDialog("Güvenlik sorusunu doğru yanıtlayın.", "error");
    refreshCowQuestion();
    return;
  }
  const milkable = state.gameplay.cows.filter((cow) => cowCanBeMilked(cow));
  if (!milkable.length) {
    return showDialog("Sağılabilir ineğin yok.", "error");
  }
  const now = Date.now();
  let total = 0;
  ...
  state.gameplay.inventory.sut = (state.gameplay.inventory.sut || 0) + total;
  renderInventory();
  renderCows();
  showDialog(
    `Toplu sağım tamamlandı. ${total.toLocaleString("tr-TR")} litre süt kazandın.`,
    "success"
  );
  saveState();
}

function cowCooldownRemaining() {
  const now = Date.now();
  const base = state.gameplay.cowPurchaseAt || 0;
  const duration = (state.gameplay.vipActive ? 30 : 60) * 60 * 1000;
  const ends = base + duration;
  return Math.max(0, ends - now);
}

function renderCowShop() {
  if (!dom.cowShopView) return;
  if (dom.cowTypes && !dom.cowTypes.dataset.rendered) {
    dom.cowTypes.innerHTML = COW_TYPES.map((type) => {
      const chance = Math.round(type.chance * 100);
      return `
        <div class="cow-type-card">
          <div class="cow-visual">${type.icon}</div>
          <div>
            <h4>${escapeHtml(type.name)} (${chance}%)</h4>
            <p>Üretim: ${type.range[0].toLocaleString("tr-TR")} / ${type.range[1].toLocaleString("tr-TR")} Litre Arası Süt</p>
          </div>
        </div>
      `;
    }).join("");
    dom.cowTypes.dataset.rendered = "true";
  }

  const remaining = cowCooldownRemaining();
  const hasCooldown = remaining > 0;
  if (dom.cowShopCooldown) {
    if (hasCooldown) {
      const minutes = Math.floor((remaining / 1000 / 60) % 60);
      const seconds = Math.floor((remaining / 1000) % 60);
      const hours = Math.floor(remaining / (1000 * 60 * 60));
      dom.cowShopCooldown.textContent = `Yeni inek almak için sürenin bitmesini beklemelisiniz! ${String(
        hours
      ).padStart(2, "0")}:${String(minutes).padStart(2, "0")}:${String(seconds).padStart(2, "0")}`;
      dom.cowShopCooldown.classList.remove("hidden");
    } else {
      dom.cowShopCooldown.classList.add("hidden");
    }
  }
  if (dom.cowPurchaseActions) {
    dom.cowPurchaseActions.classList.toggle("hidden", hasCooldown);
  }
  if (dom.cowBuyGem) dom.cowBuyGem.disabled = hasCooldown;
  if (dom.cowBuyTcoin) dom.cowBuyTcoin.disabled = hasCooldown;
}

function handleCowPurchase(premium = false) {
  const { used, capacity } = farmUsage();
  if (used + COW_SLOT_COST > capacity) {
    return showDialog("Tarlanızda yeterli yer yok.", "error");
  }
  const remaining = cowCooldownRemaining();
  if (remaining > 0) {
    return showDialog("Yeni inek almak için sürenin bitmesini beklemelisiniz!", "error");
  }
  const tcoin = state.gameplay.currencies.tcoin?.holdings || 0;
  if (tcoin < 1000) {
    return showDialog("Yeterli Tcoin yok.", "error");
  ...
  if (premium && diamonds < 10) {
    return showDialog("Yeterli elmas yok.", "error");
  }

  const chanceMap = COW_TYPES.map((type) => {
    if (premium && type.id === "montbeliard") {
      return { ...type, chance: 0.5 };
    }
    return type;
  });
  const totalChance = chanceMap.reduce((acc, type) => acc + type.chance, 0);
  const roll = Math.random() * totalChance;
  let accumulator = 0;
  let awarded = chanceMap[0];
  for (const type of chanceMap) {
    accumulator += type.chance;
    if (roll <= accumulator) {
      awarded = type;
      break;
    }
  ...
  state.gameplay.currencies.tcoin.holdings = tcoin - 1000;
  if (premium) {
    state.gameplay.balances.diamonds = diamonds - 10;
  }

  const now = Date.now();
  state.gameplay.cows.push({
    id: createCowId(),
    type: awarded.id,
    acquiredAt: now,
    expiresAt: now + COW_LIFETIME,
    grazingUntil: null,
    lastMilkAt: null,
  });
  state.gameplay.cowPurchaseAt = now;
  saveState();
  renderCurrencies();
  renderCows();
  showDialog(
    `Tebrikler! ${awarded.name} isimli inek kazandınız.`,
    "success"
  );
  showFarmView("cows");
}
...
function attachEvents() {
  ...
  if (dom.cowBuyTcoin)
    dom.cowBuyTcoin.addEventListener("click", () => handleCowPurchase(false));
